...
class Retailer(Parent):
    def __init__(self, initialInventory, stateLength1, stateLength2, actionLength, parentActionLength, xLength, type, unitTransshipmentRevenue, unitTransshipmentCost, 
        ...
        if type == 5:  # hetero maximax q-learning
            self.q_balanced_new = np.zeros((stateLength1, actionLength, parentActionLength))                                   # initialize to zeros
            self.q_cnt = np.zeros((stateLength1, actionLength))
    ...
    def InventoryUpdate_balancedGap_new(self, thisRealizedDemand, realizedSupplyLoss, getTransshipmentFrom, sendTransshipmentTo):
        # balanced gap algorithm
        self.inventoryLevel = max(self.inventoryLevel 
                                    + max(self.orderAmount - realizedSupplyLoss, 0)
                                    + getTransshipmentFrom
                                    - min(self.inventoryLevel, sendTransshipmentTo)
                                    - thisRealizedDemand, 0)

            
    def GetInventoryUpdate_balancedGap_new(self, thisRealizedDemand, realizedSupplyLoss, getTransshipmentFrom, sendTransshipmentTo):
        # balanced gap algorithm
        inventoryLevel = max(self.inventoryLevel 
                                    + max(self.orderAmount - realizedSupplyLoss, 0)
                                    + getTransshipmentFrom
    ...
    def GetRealizedReward_balancedGap_new(self, thisRealizedDemand, realizedSupplyLoss, getTransshipmentFrom, sendTransshipmentTo, unitTransRevenueAnotherRetailer):
        realizedReward = ((self.unitRevenue * min(thisRealizedDemand, self.inventoryLevel + max(self.orderAmount - realizedSupplyLoss, 0) + getTransshipmentFrom - min(self.inventoryLevel, sendTransshipmentTo))) 
                          - (self.unitOrderingCost * self.orderAmount) 
                          - (self.unitHoldingCost * self.inventoryLevel) 
                          - (self.unitPenaltyforUnmetDemand * max(thisRealizedDemand - (self.inventoryLevel + max(self.orderAmount - realizedSupplyLoss, 0) + getTransshipmentFrom - min(self.inventoryLevel, sendTransshipmentTo)), 0)) 
                          - (unitTransRevenueAnotherRetailer * (getTransshipmentFrom)) 
                          - (self.unitTransshipmentCost * min(self.inventoryLevel, sendTransshipmentTo)) 
                          + (self.unitTransshipmentRevenue * min(self.inventoryLevel, sendTransshipmentTo)))
    ...
    def GetUnmetDemand_balancedGap_new(self, thisRealizedDemand, realizedSupplyLoss, getTransshipmentFrom, sendTransshipmentTo, unitTransRevenueAnotherRetailer):
        unmetDemand = max(thisRealizedDemand - (self.inventoryLevel + max(self.orderAmount - realizedSupplyLoss, 0) + getTransshipmentFrom - min(self.inventoryLevel, sendTransshipmentTo)), 0)
