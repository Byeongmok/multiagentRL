...
class Sim():
    ...
    def InitializePi_balancedGap_New(self, stateLength1, stateLength2, actionLength1, actionLength2, actionOLength):
        self.pi1_hetero = np.ones(shape=[stateLength1, actionLength1])
        self.pi1_hetero = self.pi1_hetero / (actionLength1)   
        
        self.pi2_hetero = np.ones(shape=[stateLength2, actionLength2])
        self.pi2_hetero = self.pi2_hetero / (actionLength2)   
    ...
    def GetIndexFromInverseCumulativePi_balancedGap_New(self, stateIndex1, stateIndex2, probability1, probability2, probability3, a1, a2, o):
        ...
        for i in range(len(a1.action)):
            sum += self.pi1_hetero[stateIndex1, i]
        ...
        for i in range(len(a2.action)):
            sum += self.pi2_hetero[stateIndex2, i]
    ...
    def GetStateIndex(self, inventory1, inventory2, s):
        ...
        if(not flag2):
            if inventory2 >= s.state[0][len(s.state) - 1][1]:
    ...
    def GetMaxDiscountedFutureQValue_balancedGap_new(self, retailer, retailerIndex, a, o, nextStateIndex1, nextStateIndex2):
        ...
        if retailerIndex == 1:
            ...
            for j in range(len(o.action)):
                sum = 0.0
                for i in range(len(a.action)):
                    sum += (retailer.q_balanced_new[nextStateIndex1, i, j] * self.pi1_hetero[nextStateIndex1, i])
                q_valueList.append(sum)
            return np.max(q_valueList)        
        ...
        if retailerIndex == 2:
            for j in range(len(o.action)):
                sum = 0.0
                for i in range(len(a.action)):
                    sum += (retailer.q_balanced_new[nextStateIndex2, i, j] * self.pi2_hetero[nextStateIndex2, i])
                q_valueList.append(sum)
            return np.max(q_valueList)        
    ...
    def GetAverageAndStdDev_balancedGap_new_retailer(self, currentStateIndex, Q):
        average = 0
        stdDev = 0
    ...
    def UpdatePi_balancedGap_new(self, parent, retailer1, retailer2,  currentStateIndex1, currentStateIndex2, a1Length, a2Length, oLength):
        ...
        meanStdDev1 = self.GetAverageAndStdDev_balancedGap_new_retailer(currentStateIndex1, retailer1.q_balanced_new)
        meanStdDev2 = self.GetAverageAndStdDev_balancedGap_new_retailer(currentStateIndex2, retailer2.q_balanced_new)
        ...
        for i in range(a1Length):
            for j in range(oLength):
                denominator += np.exp(self.N(retailer1.q_balanced_new[currentStateIndex1, i, j], meanStdDev1))
                    
        
        for i in range(a1Length):
            sum = 0.0           
            for j in range(oLength):
                sum += np.exp(self.N(retailer1.q_balanced_new[currentStateIndex1, i, j], meanStdDev1))
                
            self.pi1_hetero[currentStateIndex1, i] = sum / denominator                        
    
    
        denominator = 0.0
        for i in range(a2Length):
            for j in range(oLength):
                denominator += np.exp(self.N(retailer2.q_balanced_new[currentStateIndex2, i, j], meanStdDev2))
                    
                    
        for i in range(a2Length):
            sum = 0.0
            for j in range(oLength):
                sum += np.exp(self.N(retailer2.q_balanced_new[currentStateIndex2, i, j], meanStdDev2))                    
    
            self.pi2_hetero[currentStateIndex2, i] = sum / denominator                        
    ...
    def Run_balancedGap_New(self, maxT, demand1, demand2, supplyLoss1, supplyLoss2, retailer1, retailer2, parent, a1, a2, o, s,
        ...
        while(t < maxT):
            ...
            # actual transshipment amount
            getTransshipmentFrom2 = min(retailer2.inventoryLevel, o.action[self.o_Index][1])
            getTransshipmentFrom1 = min(retailer1.inventoryLevel, o.action[self.o_Index][0])
            
            
            # get R1(s, a1, o)
            currentReward1 = retailer1.GetRealizedReward_balancedGap_new(demand1.realizedArray[t], supplyLoss1.realizedArray[t], getTransshipmentFrom2, 
                                                         o.action[self.o_Index][0], retailer2.unitTransshipmentRevenue)
            
            # get R2(s, a2, o)
            currentReward2 = retailer2.GetRealizedReward_balancedGap_new(demand2.realizedArray[t], supplyLoss2.realizedArray[t], getTransshipmentFrom1, 
                                                         o.action[self.o_Index][1], retailer1.unitTransshipmentRevenue)                                                         
            
            
            # get next inventory level
            nextInventoryLevel1 = retailer1.GetInventoryUpdate_balancedGap_new(demand1.realizedArray[t], supplyLoss1.realizedArray[t], getTransshipmentFrom2, o.action[self.o_Index][0])
            nextInventoryLevel2 = retailer2.GetInventoryUpdate_balancedGap_new(demand2.realizedArray[t], supplyLoss2.realizedArray[t], getTransshipmentFrom1, o.action[self.o_Index][1])
            ...
            # update Q1(s, a1, o)
            retailer1.q_balanced_new[currentStateIndex1, self.a1_Index, self.o_Index] = (((1.0 - self.learningRate) * retailer1.q_balanced_new[currentStateIndex1, self.a1_Index, self.o_Index]) 
            ...
            # update Q2(s, a2, o)
            retailer2.q_balanced_new[currentStateIndex2, self.a2_Index, self.o_Index] = (((1.0 - self.learningRate) * retailer2.q_balanced_new[currentStateIndex2, self.a2_Index, self.o_Index]) 
            ...
            # inventory update
            retailer1.InventoryUpdate_balancedGap_new(demand1.realizedArray[t], supplyLoss1.realizedArray[t], getTransshipmentFrom2, o.action[self.o_Index][0])           
            retailer2.InventoryUpdate_balancedGap_new(demand2.realizedArray[t], supplyLoss2.realizedArray[t], getTransshipmentFrom1, o.action[self.o_Index][1])
    ...
    def Test_balancedGap_New(self, maxT, demand1, demand2, supplyLoss1, supplyLoss2, retailer1, retailer2, parent, a1, a2, o, s, seedRndPi, initialInventoryLevel_1, initialInventoryLevel_2):
        ...
        while(t < maxT):
            ...
            # get UnmetDemand
            unmetDemand1 = retailer1.GetUnmetDemand_balancedGap_new(demand1.realizedArray[t], supplyLoss1.realizedArray[t], getTransshipmentFrom2, 
                                                         o.action[self.o_Index][0], retailer2.unitTransshipmentRevenue)
            
            unmetDemand2 = retailer2.GetUnmetDemand_balancedGap_new(demand2.realizedArray[t], supplyLoss2.realizedArray[t], getTransshipmentFrom1, 
                                                         o.action[self.o_Index][1], retailer1.unitTransshipmentRevenue)           
